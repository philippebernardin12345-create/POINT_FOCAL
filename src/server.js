...
async function startServer() {
  try {
    ...
    logger.info(
      `[Startup] Registre chargé : ${registry.list().length} modules actifs.`
    );

    // 2. Démarrer le serveur HTTP
    app.listen(PORT, () => {
      logger.info(
        `[Startup] Point Focal Backend V10.4 démarré sur le port ${PORT}`
      );
      logger.info(
        `[Startup] Environnement : ${process.env.NODE_ENV || "development"}`
      );
      logger.info(
        `[Startup] API disponible : http://localhost:${PORT}/api`
      );
    });
