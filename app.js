// app.js — Authentification et Workflow V3 Compléts avec Interaction Active YouTube
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ACTION LOGIQUE DE RÉCUPÉRATION DU MOT DE PASSE
    ...
    // LOGIQUE CAPTIVE : L'utilisateur ouvre YouTube mais le chrono tourne sur Point Focal
    let chronoDemarre = false;
    let tempsVisionnage = 10; // Valeur de test (À remplacer par 180 pour 3 minutes en production)
    let intervalleVisionnage;

    fausseVideoYt.addEventListener("click", () => {
        // ÉTAPE 1 : On ouvre ta VRAIE vidéo YouTube dans un nouvel onglet
        // L'utilisateur atterrit sur l'application YouTube officielle : il peut LIKER, S'ABONNER et COMMENTER !
        window.open("https://youtu.be/9uPybhkqYw4", "_blank");

        // ÉTAPE 2 : On démarre instantanément le chrono Point Focal resté en arrière-plan
        if (!chronoDemarre) {
            chronoDemarre = true;
            ...
            intervalleVisionnage = setInterval(() => {
                ...
                if (tempsVisionnage <= 0) {
                    ...
                    // Alerte sonore/visuelle système pour forcer le retour de l'utilisateur s'il a oublié
    ...
    btnVerifierPaiement.addEventListener("click", () => {
        ...
        setTimeout(() => {
            ...
            alert("Succès ! Inscription validée.\n\nVotre licence Point Focal V3 est désormais active !");
        }, 2000);
    });
    ...
    btnDeconnexion.forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            chronoDemarre = false; tempsVisionnage = 10; timerVisuel.textContent = "10s";
